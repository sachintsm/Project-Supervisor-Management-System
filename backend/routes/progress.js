...
router.post("/addtask", verify, async (req, res, next) => {
  try {
    ...
    const studentIds = await User.find({ indexNumber: groupMembers }).select(
      "_id"
    );
    let studentList = [];
    let progressList = [];
...
router.patch("/edittask/:id", verify, async (req, res, next) => {
  try {
    ...
    const obj = req.body;
    const result = await ProgressTasks.findByIdAndUpdate(id, obj, {
      new: true,
    });
    res.send(result);
  } catch (e) {
...
router.get("/gettasks/:id", async (req, res, next) => {
  try {
    ...
    const result = await ProgressTasks.find({ groupId: groupId }).sort({
      totalProgress: -1,
    });
    res.send(result);
  } catch (e) {
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      for (let i in tasks) {
        let index = -1;
        for (let j in tasks[i].studentList) {
          ...
          if (tasks[i].studentList[j] == userId._id) {
            ...
            let individualContribution =
              individualProgress * tasks[i].taskWeight; // calculating individual contribution
...
router.post("/getstudenttaskprogress/:studentIndex", async (req, res, next) => {
  try {
    ...
    const userId = await User.findOne({ indexNumber: index }).select("_id");
    const task = await ProgressTasks.findOne({
      studentList: userId._id,
      _id: taskId,
    });
    let studentProgress = 0;
    for (let j in task.studentList) {
      //get the array index of student
...
router.post("/addprogressupdate", verify, async (req, res, next) => {
  try {
    ...
    if (newProgress < 0) {
      // if student reduce progress more than his current progress
      newProgress = 0;
    ...
    const result = await ProgressTasks.findByIdAndUpdate(taskId, obj, {
      new: true,
    });
...
router.get("/getprojectprogressupdates/:groupId", async (req, res, next) => {
  try {
    const groupId = req.params.groupId;
    const result = await ProgressUpdates.find({ groupId: groupId }).sort({
      timestamp: -1,
    });
    res.send(result);
  } catch (e) {
...
router.get("/gettaskprogressupdates/:taskId", async (req, res, next) => {
  try {
    ...
    const result = await ProgressUpdates.find({ taskId: taskId }).sort({
      timestamp: -1,
    });
    res.send(result);
  } catch (e) {
