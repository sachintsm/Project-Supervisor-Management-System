...
router.post("/projecttype", async (req, res, next) => {
  try {
    ...
    if (
      !(
        req.body.isFirstYear ||
        req.body.isSecondYear ||
        req.body.isThirdYear ||
        req.body.isFourthYear
      )
    ) {
...
router.post("/", async (req, res, next) => {
  try {
    ...
    const isProjectExist = await Projects.find({
...
router.patch("/:id", async (req, res, next) => {
  try {
    ...
    const isProjectExist = await Projects.findOne({
      projectYear: update.projectYear,
    ...
    if (isProjectExist && isProjectExist._id != id) {
      ...
      // if all students are registered
        ...
        const result = await Projects.findByIdAndUpdate(id, update, {
          new: true,
        });
        res.send(result);
      }
...
router.patch("/projecttype/:id", async (req, res, next) => {
  try {
    ...
    const result = await ProjectType.findByIdAndUpdate(id, update, {
      new: true,
    });
    res.send(result);
  } catch (err) {
...
router.get("/", async (req, res, next) => {
  try {
    const projects = await Projects.find({ isDeleted: false }).sort({
      projectYear: -1,
    });
    res.send(projects);
  } catch (err) {
...
router.get("/studentprojects/:studentId", async (req, res, next) => {
  try {
    ...
    if (result.length == 0) {
      ...
      const projectList = await CreateGroups.find({
        groupMembers: index,
      }).select("projectId");
      let projectIdList = [];
...
router.post("/addSupervisor", verify, async (req, res) => {
  const existSupervisor = await Projects.findOne({
    _id: req.body.projectId,
    supervisorList: req.body.supervisors,
  });
  if (existSupervisor)
    return res.json({ state: false, msg: "Already Exists..!" });
  else {
    Projects.find({ _id: req.body.projectId })
      .update({
...
router.post("/deletesupervisorGroup", verify, async (req, res) => {
  ...
  await Projects.find({ _id: projectId })
    .update({ $pull: { supervisorList: supervisor } })
    .exec()
    .then((data) => {
...
router.get("/getAllActiveProjectDataS/:id", async (req, res) => {
  const coId = req.params.id;
  await Projects.find({ supervisorList: coId, projectState: true })
