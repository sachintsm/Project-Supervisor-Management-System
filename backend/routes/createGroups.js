...
router.post("/add", async (req, res) => {
  const maxNumber = await CreateGroups.find({
...
router.get("/get/:projectId", (req, res) => {
  const projectId = req.params.projectId;
  CreateGroups.find({ projectId: projectId })
    .exec()
    .then((data) => {
      res.json({ state: true, data: data, msg: "Data successfully sent..!" });
    })
...
router.post("/addStudentIndex", async (req, res) => {
  ...
  if (existing)
    return res.json({ state: false, msg: "This Index is already added..!" });
  else {
    CreateGroups.find({ _id: id })
      .update({ $push: { groupMembers: index } })
      .exec()
      .then((data) => {
...
router.post("/removeStudentIndex", async (req, res) => {
  ...
  CreateGroups.find({ _id: id })
    .update({ $pull: { groupMembers: index } })
    .exec()
    .then((data) => {
...
router.post("/getsupervisorGroup", async (req, res) => {
  ...
  CreateGroups.find({ projectId: projectId, supervisors: supervisor })
    .exec()
    .then((data) => {
      res.json({ state: true, data: data, msg: "Data successfully sent..!" });
    })
...
router.post("/remove-supervisor", verify, async (req, res) => {
  ...
  await CreateGroups.find({ projectId: projectId, groupId: groupId })
    .update({ $pull: { supervisors: supervisorId } })
    .exec()
    .then((data) => {
      res.json({ state: true, msg: "Index successfully deleted..!" });
    })
...
router.post("/active&groups", async (req, res) => {
  ...
  await CreateGroups.find({
    ...
    .then((data) => {
      res.json({
        state: true,
        msg: "Data successfully Transfered..!",
        data: data,
      });
    })
    .catch((err) => {
...
router.get("/groupCount/:id", async (req, res) => {
  ...
  await CreateGroups.find({ projectId: projectId, groupState: true })
    .count()
    .exec()
    .then((data) => {
      res.json({
        state: true,
        msg: "Data Transfered Successfully..!",
        data: data,
      });
    })
    .catch((error) => {
...
router.post("/groupDetails/:studentId", async (req, res, next) => {
  try {
    ...
    const result = await CreateGroups.findOne({
      groupMembers: indexNumber.indexNumber,
      projectId: projectId,
    });
...
router.get("/groupformnotification/:studentId", async (req, res, next) => {
  try {
    ...
    const groups = await CreateGroups.find(
      { groupMembers: index.indexNumber },
      { _id: false }
    ).select("projectId");
...
router.post("/allgrouprequest/:userId", async (req, res, next) => {
  try {
    ...
    let result1 = await GroupRequests.findOne({
      pendingList: userIndex.indexNumber,
      projectId: projectId,
    });
    let result2 = await GroupRequests.findOne({
      acceptedList: userIndex.indexNumber,
      projectId: projectId,
    });
    let result = null;
    if (result1) {
...
router.post("/grouprequest/:userId", async (req, res, next) => {
  try {
    ...
    let userIndex = await User.findOne({ _id: userId }).select("indexNumber");
    let result = await GroupRequests.findOne({
      projectId: projectId,
      allStudentList: userIndex.indexNumber,
    });
    res.send(result);
  } catch (e) {
    console.log(e);
...
router.patch("/grouprequest/:id", async (req, res, next) => {
  try {
    const id = req.params.id;
    let result = await GroupRequests.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    res.send(result);
  } catch (e) {
...
router.get("/coordinatorgrouprequests/:userId", async (req, res, next) => {
  try {
    ...
    const groupRequests = await GroupRequests.find({
      pendingList: [],
      declinedList: [],
...
router.patch("/groupDetails/:groupId", async (req, res, next) => {
  try {
    ...
    const result = await CreateGroups.findByIdAndUpdate(id, update, {
      new: true,
    });
    res.send(result);
  } catch (err) {
