...
router.get("/getBiweeklyLink/:_id", async (req, res) => {
  ...
  biweekly
    .find({ projectId: projectId, isDeleted: false })
    .sort({ date: -1 } && { time: -1 })
    .then((data) => {
      res.send({
        state: true,
...
router.patch("/deleteBiweekly/:_id", async (req, res, next) => {
  try {
    ...
    console.log(proID);
    const result = await biweekly.findByIdAndUpdate(
      proID,
      { isDeleted: true },
      { new: true }
    );
...
router.get("/getBiweeklyNumber/:id", async (req, res) => {
  try {
    ...
    const biweeklyLinks = await biweekly.find({
      projectId: projectId,
      isDeleted: false,
    });
    ...
    res.send({
      state: true,
      data: data,
      msg: "Data Transfering successfull..!",
    });
  } catch (error) {
    console.log(error);
