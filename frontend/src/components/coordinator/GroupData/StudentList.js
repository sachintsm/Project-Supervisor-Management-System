...
class StudentList extends Component {
  ...
  componentDidMount = async () => {
    ...
    await axios
      .get(backendURI.url + "/users/studentList/" + this.props.obj, {
        headers: headers,
      })
      .then((res) => {
        // console.log(res)
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            ...
            axios
              .post(backendURI.url + "/createGroups/removeStudentIndex", data)
              .then((res) => {
        ...
        {this.state.studentList.map((user, index) => {
          return (
            <div key={user.id} className="sd-sl-card-student">
              <Row>
                <Col md="3" xs="6">
                  <span className="stu-index-data">
                    {" "}
                    <Index id={user.id} index={user.index} />
                  </span>
                ...
                <Col md="1" xs="2">
                  <DeleteForeverIcon
                    className="gd-del-btn"
                    onClick={() => this.delete(user.index)}
                  />
                </Col>
              </Row>
            </div>
