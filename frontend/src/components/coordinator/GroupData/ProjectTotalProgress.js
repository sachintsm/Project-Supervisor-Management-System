    ...
    return (
      <Row>
        <Col md={12} className="total-progress-card-div-co">
          <Card
            className="total-progress-card"
            onClick={() => this.individualProgressTask()}
          >
                ...
                <Col md={4} sm={12} xs={12} className="circular-progress-div">
                  {this.state.totalProgressInt < 51 && (
                    <CircularProgressbar
                      styles={buildStyles({
                        textColor: `rgb(150,${
                          this.state.totalProgressInt * (255 / 50) - 100
                        },0)`,
                        pathColor: `rgb(255,${
                          this.state.totalProgressInt * (255 / 50)
                        },0)`,
                      })}
                  ...
                  {this.state.totalProgressInt > 50 && (
                    <CircularProgressbar
                      styles={buildStyles({
                        textColor: `rgb(${
                          255 -
                          (this.state.totalProgressInt - 50) * (255 / 50) -
                          100
                        },150,0)`,
                        pathColor: `rgb(${
                          255 - (this.state.totalProgressInt - 50) * (255 / 50)
                        },255,0)`,
                      })}
                      value={this.state.totalProgress}
                      text={`${this.state.totalProgressInt}%`}
