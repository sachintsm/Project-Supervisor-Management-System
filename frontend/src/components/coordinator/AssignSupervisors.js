...
class AssignSupervisors extends Component {
  ...
  async componentDidMount() {
    ...
    axios
      .get(backendURI.url + "/projects/active&projects/" + coId.id)
      .then((res) => {
        this.setState({
    ...
    //load staff members
    await axios
      .get(backendURI.url + "/users/stafflistwithguest", { headers: headers })
      .then((result) => {
        ...
        buttons: [
          {
            ...
            onClick: async () => {
              ...
              for (let i = 0; i < selectedStaff.length; i++) {
                ...
                await axios
                  .post(backendURI.url + "/projects/addSupervisor", data, {
                    headers: headers,
                  })
                  .then(async (res) => {
                    if (res.data.state === false) {
                      ...
                      await axios
                        .get(
                          backendURI.url +
                            "/users/updateSupervisor/" +
                            selectedStaff[i].value
                        )
                        .then((res) => {
                          if (res.data.state === false) {
                            this.setState({
  ...
  async searchGroups() {
    if (this.state.projectId === "") {
      ...
      await axios
        .get(
          backendURI.url + "/projects/getSupervisors/" + this.state.projectId,
          { headers: headers }
        )
        .then((res) => {
          this.setState({
      ...
      for (let i = 0; i < this.state.supervisorIdList.length; i++) {
        ...
        await axios
          .get(backendURI.url + "/users/getUserName/" + id)
          .then(async (res) => {
            let name =
            ...
            await axios
              .post(backendURI.url + "/createGroups/getsupervisorGroup", data)
              .then((res) => {
  ...
  async deleteSupervisor(data, groups) {
    ...
    await axios
      .post(backendURI.url + "/createGroups/getsupervisorGroup", dt)
      .then((res) => {
        for (let j = 0; j < res.data.data.length; j++) {
          var group = res.data.data[j].groupId;
          array1.push(group);
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            ...
            await axios
              .post(backendURI.url + "/projects/deletesupervisorGroup", dt, {
                headers: headers,
              })
              .then((res) => {});
            for (let j = 0; j < array1.length; j++) {
              ...
              await axios
                .post(
                  backendURI.url + "/createGroups/remove-supervisor",
                  data,
                  { headers: headers }
                )
                .then((res) => {
  ...
  render() {
    const {
      ...
      spinnerDiv2,
      staffOptionList,
    } = this.state; //load the projects to the dropdown menu of the coordinator
    ...
    let list =
      staffOptionList.length > 0 &&
      staffOptionList.map((item, i) => {
        return (
          <div key={i}>
            <Row style={{ marginBottom: "2px" }}>
              <Col md={1}>
                {item.isGuest && <p className="guestSup">Guest</p>}
                {item.isStaff && <p className="staffSup">Staff</p>}
              </Col>
              <Col md={3}>
                ...
                <Row>
                  <Col md="10" xs="12">
                    <div className="form-group as-dropdown-select">
                      <select
                        className="form-control as-dropdown-select"
                        id="dropdown"
