...
class PresentationFeedback extends Component {
  ...
  async searchGroups() {
    ...
    await axios
      .post(backendURI.url + "/presentationFeedback/getProjectFeedback", data, {
        headers: headers,
      })
      .then((res) => {
    ...
    for (let i = 0; i < this.state.allFeedback.length; i++) {
      ...
      for (let j = i; j < this.state.allFeedback.length; j++) {
        if (
          this.state.allFeedback[i].groupId ===
          this.state.allFeedback[j].groupId
        ) {
          let fullName = "";
          await axios
            .get(
      ...
      const found = this.state.finalBlock.some(
        (el) => el.groupId === this.state.allFeedback[i].groupId
      );
      if (!found) {
        let groupNumber = "";
        await axios
        ...
        const pfblock = new PresentationFeedbackBlock(
          this.state.allFeedback[i].groupId,
          groupNumber,
        ...
        <div className="container pf-container-div">
          ...
          <div className="card pg-search-card">
            <div className="container">
              <div className="container">
                <p className="pg-reg-head">
                  Project Groups Presentation Feedbacks{" "}
                </p>
                  ...
                  <Col md="5" xs="12">
                    <div className="form-group pg-dropdown-select">
                      <select
                        className="form-control pg-dropdown-select"
                        id="dropdown"
                        onChange={this.handleDropdownPresentationChange}
                      >
