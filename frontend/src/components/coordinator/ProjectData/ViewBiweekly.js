...
export class ViewBiweekly extends Component {
  ...
  getBiweekSubmissions = () => {
    ...
    axios
      .get(
        backendURI.url +
          "/biweeksubmissions/getbiweeklinksubmissions/" +
          this.state.biweeklyData._id,
        { headers: headers }
      )
      .then((res) => {
        this.setState({
      ...
      <div className="biweekly-supervisor">
        ...
        <div style={{ minHeight: "680px", padding: "0px 80px" }}>
          <div className="title-div margin-top-30">
            <h3 className="title">
              Biweekly Report {this.state.biweeklyData.biweeklyNumber}{" "}
              (Submissions)
            </h3>
          </div>
          ...
          <Table hover style={{ marginTop: 50 }}>
            ...
            <tbody>
              {!this.state.loading &&
                this.state.biWeekSubmissions.map((item, key) => {
