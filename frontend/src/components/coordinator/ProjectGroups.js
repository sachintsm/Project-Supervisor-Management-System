...
class ProjectGroups extends Component {
  ...
  async searchGroups() {
    ...
    await axios
      .get(backendURI.url + "/createGroups/get/" + this.state.projectId)
      .then((res) => {
        this.setState({
        ...
        for (let j = 0; j < this.state.groupData[i].supervisors.length; j++) {
          await axios
            .get(
              backendURI.url +
                "/users/getUserName/" +
                this.state.groupData[i].supervisors[j]
            )
            .then((res) => {
              var supervisorName =
                      ...
                      {this.state.groupDataBlock.map((item) => {
                        return (
                          <tr
                            ...
                            key={item.groupId}
                            onClick={() => this.groupDataHandler(item._id)}
                          >
                            <td className="table-body tbl-item">
                              {item.groupId}
                            </td>
                            <td className="table-body tbl-item">
                              {item.groupEmail}
                            ...
                            <td style={{ textAlign: "center" }}>
                              ...
                              >
                                <DeleteForeverIcon
                                  className="del-btn"
                                  fontSize="default"
                                  onClick={() => this.deleteGroup(item._id)}
                                />
                              </span>
                            </td>
                          </tr>
