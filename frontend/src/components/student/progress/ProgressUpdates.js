...
class ProgressUpdates extends Component {
  ...
  getUserName = () => {
    this.state.progressUpdates.map((item) => {
      ...
      axios
        .get(backendURI.url + "/users/getUser/" + item.userId, {
          headers: headers,
        })
        .then((res) => {
  ...
  getTaskTitle = () => {
    this.state.progressUpdates.map((item) => {
      ...
      axios
        .get(backendURI.url + "/progress/gettaskdetails/" + item.taskId, {
          headers: headers,
        })
        .then((res) => {
          ...
          this.state.progressUpdates.map((item) => {
            return (
              <Card
                ...
                key={item._id}
                style={{
                  borderColor: item.progressChange < 0 ? "red" : "#27d600",
                  borderWidth: "1.1px",
                }}
              >
                <Row>
