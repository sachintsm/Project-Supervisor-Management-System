...
class TaskCard extends Component {
  ...
  editTask = () => {
    this.props.history.push("/studenthome/viewproject/progresstasks/viewtask", {
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            ...
            axios
              .delete(
                backendURI.url + "/progress/deletetask/" + this.state.task._id,
                { headers: headers }
              )
              .then((res) => {
                console.log(res);
          ...
          <Card className="task-card" onClick={this.editTask}>
            {this.state.projectDetails.projectState && (
              <div>
                <Card.Header className="card-header">
                  {this.state.task.taskTitle} ({this.state.task.totalProgress}%)
            ...
            <Card.Body className="card-body">
              <div className="circular-progress-div">
                {this.state.task.totalProgress <= 33 && (
                  <CircularProgressbar
                    styles={buildStyles({
                      textColor: "#DC3545",
                      pathColor: "#DC3545",
                    })}
                    value={this.state.task.totalProgress}
                    text={`${this.state.task.totalProgress}%`}
                  />
                )}
                {this.state.task.totalProgress <= 66 &&
                  this.state.task.totalProgress > 33 && (
                    <CircularProgressbar
                      styles={buildStyles({
                        textColor: "#FFC107",
                ...
                {this.state.task.totalProgress <= 100 &&
                  this.state.task.totalProgress > 66 && (
                    <CircularProgressbar
                      styles={buildStyles({
                        textColor: "#28A745",
              ...
              <Card.Text>
                <Slider
                  disabled
                  defaultValue={this.state.task.taskWeight}
