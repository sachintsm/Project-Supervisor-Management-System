...
class TaskProgressCard extends Component {
  ...
  componentDidMount() {
    this.setState({
      ...
      totalProgressInt:
        Math.round(this.state.taskDetails.totalProgress * 1) / 1,
      loading: false,
    });
  }
          ...
          <Col md={12} className="total-progress-card-div">
            <Card className="total-progress-card">
              <Card.Header className="card-header">
                {this.state.taskDetails.taskTitle} (
                {this.state.taskDetails.totalProgress}%)
              </Card.Header>
              <Card.Body className="card-body">
                <Row>
                  <Col md={4} sm={12} xs={12} className="circular-progress-div">
                    {this.state.totalProgress <= 33 && (
                      <CircularProgressbar
                        styles={buildStyles({
                          textColor: "#DC3545",
                          pathColor: "#DC3545",
                        ...
                        text={`${this.state.totalProgress}%`}
                      />
                    )}
                    {this.state.totalProgress <= 66 &&
                      this.state.totalProgress > 33 && (
                        <CircularProgressbar
                          styles={buildStyles({
                            textColor: "#FFC107",
