...
function CenteredModal(props) {
  ...
  return (
    ...
    >
      ...
      <Modal.Footer>
        <Button onClick={re} disabled={!des}>
          Send
        </Button>
        <Button onClick={hide}>Close</Button>
      </Modal.Footer>
...
const Staff = React.memo((props) => (
  <tr>
    ...
    <td>
      <a href="#" onClick={() => props.view(props.staff._id, props.result)}>
        {props.staff.email}
      </a>
    </td>
    <td>
      <ButtonToolbar>
        <Button
          ...
          onClick={() => props.view(props.staff._id)}
        >
          Projects
        </Button>
        <ProjectModal
    ...
    <td>
      <ButtonToolbar>
        <Button
          ...
          value="Mod"
          className="btn btn-info"
          onClick={() => props.send(props.staff._id)}
        >
          Request
        </Button>
        <CenteredModal
...
const ReqState = React.memo((props) => (
  <tr>
    <td>
      {props.req.supFirstName} {props.req.supLastName}
    </td>
...
export default class Profile extends Component {
  ...
  async componentDidMount() {
    ...
    axios
      .get(backendURI.url + "/users/getSup/" + this.state.project._id)
      .then((response) => {
    ...
    axios
      .get(backendURI.url + "/users/getReqStatus/" + userData.id)
      .then((response) => {
        console.log(response.data.data);
        ...
        axios
          .post(
            backendURI.url + "/users/getStatus/" + this.state.project._id,
            obj
          )
          .then((response) => {
            console.log(response.data.data);
  ...
  UserList4() {
    let filteredUsers = this.state.userS.filter((currentStaff) => {
    ...
    return filteredUsers.map((currentStaff, i) => {
      ...
      return (
        <Staff
          request={this.requestSup}
          send={this.reqSend}
          view={this.viewPro}
          result={this.state.pro}
          hiden={this.hideModal}
          hiden2={this.hideModal2}
          desHan={this.handleChange}
          descrip={this.state.descript}
          stat={this.state.show}
          stat2={this.state.show2}
          dis={this.state.dis}
          staff={currentStaff}
          key={i}
        />
  ...
  StatusList4() {
    ...
    let filteredStatus = this.state.reqSt.filter((currentStatus) => {
  ...
  reqSend(sup) {
    ...
    axios
      .post(backendURI.url + "/users/check", obj)
      .then((res) => {
  ...
  viewPro(sup, k) {
    ...
    axios
      .get(backendURI.url + "/users/getSupPro/" + sup)
      .then((res) => {
    ...
    return (
      <ProjectModal show={this.showModal2} hide={this.hideModal2} re={k} />
    );
  }
  requestSup() {
    const userData = getFromStorage("auth-id");
      ...
      buttons: [
        {
          ...
          onClick: () => {
            ...
            console.log(obj);
            axios
              .post(backendURI.url + "/users/add", obj)
              .then((res) => {
                console.log(res.data);
                if (res.data.state === true) {
                  this.setState({
                        ...
                        <div className="row" style={{ marginTop: "20px" }}>
                          <div className="card">
                            <div>
                              ...
                              <div
                                className="container"
                                style={{ marginLeft: "24px", width: "95.5%" }}
                              >
                                <Table responsive>
                                  <thead>
                                    <tr>
