...
class FormGroups extends Component {
  ...
  getRequestDetails() {
    ...
    axios
      .post(
        backendURI.url + "/createGroups/grouprequest/" + userId,
        projectId,
        { headers: headers }
      )
      .then((res) => {
  ...
  getUserIndex = () => {
    ...
    axios
      .get(backendURI.url + "/users/get/" + userId, { headers: headers })
      .then((res) => {
  ...
  getProjectDetails = () => {
    ...
    axios
      .get(backendURI.url + "/projects/getProject/" + this.state.projectId, {
        headers: headers,
      })
      .then((res) => {
        let studentList = res.data.data.studentList.filter(
          (item) => item !== this.state.userIndex
        );
          ...
          onClick: async () => {
            if (this.state.groupRequest) {
              ...
              if (this.state.acceptedList.includes(index)) {
                this.setState({
                  acceptedList: this.state.acceptedList.filter(
                    (item) => item !== index
                  ),
              ...
              if (this.state.declinedList.includes(index)) {
                this.setState({
                  declinedList: this.state.declinedList.filter(
                    (item) => item !== index
                  ),
                  studentList: [...this.state.studentList, index],
                });
              ...
              if (this.state.pendingList.includes(index)) {
                this.setState({
                  pendingList: this.state.pendingList.filter(
                    (item) => item !== index
                  ),
                  studentList: [...this.state.studentList, index],
                });
              ...
              if (this.state.allStudentList.includes(index)) {
                this.setState({
                  allStudentList: this.state.allStudentList.filter(
                    (item) => item !== index
                  ),
                });
              }
            } else {
              let newAddedList = this.state.addedList.filter(
                (item) => item !== index
              );
              this.setState({
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            ...
            axios
              .post(backendURI.url + "/createGroups/grouprequest/", request, {
                headers: headers,
              })
              .then((res) => {
                this.setState(
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            ...
            axios
              .patch(
                backendURI.url +
                  "/createGroups/grouprequest/" +
                  this.state.groupRequest._id,
                request,
                { headers: headers }
              )
              .then((res) => {
              ...
              {!this.state.loading && (
                <Card className="project-detail-card">
                  <Card.Body className="card-body">
                    ...
                    <h5>
                      Project Year: {this.state.projectDetails.projectYear}
                    </h5>
                    <h5>
                      Project Type: {this.state.projectDetails.projectType}
                    </h5>
                    <h5>
                      Academic Year: {this.state.projectDetails.academicYear}
                    </h5>
                  </Card.Body>
                </Card>
              )}
            ...
            {!this.state.loading2 && this.state.groupRequest === null && (
              <div className="group-form-card-div">
                <Card className="group-form-card">
                        ...
                        {this.state.addedList.map((item) => {
                          return (
                            <StudentInfoCard2
                              ...
                              key={item}
                              type={"normal"}
                              selectStudent={this.removeStudent}
                            />
                          );
                        })}
                      </Row>
                  ...
                  <Row>
                    {!this.state.loading &&
                      this.state.studentList.filter((index) =>
                        index.match(this.state.searchText)
                      ).length > 0 &&
                      this.state.studentList
                        .filter((index) => index.match(this.state.searchText))
                        .map((item) => {
                          return (
                            <StudentInfoCard
                              index={item}
                ...
                <Card className="group-form-card">
                  ...
                  {!this.state.loading && this.state.allStudentList.length > 0 && (
                    <div className="added-list-div">
                      ...
                      {!this.state.loading &&
                        this.state.allStudentList.length === 1 && (
                          <div className="error-div">
                            Not students have been added
                          </div>
                        )}
