...
class SupervisorDetails extends Component {
  ...
  getSupervisorDetails = () => {
    ...
    axios
      .get(backendURI.url + "/users/getSupervisorEmail/" + this.state.index, {
        headers: headers,
      })
      .then((res) => {
        this.setState({
  ...
  getCurrentProjects = () => {
    ...
    axios
      .get(
        backendURI.url +
          "/supervisorrequests/getcurrentprojectlist/" +
          this.state.index,
        { headers: headers }
      )
      .then((res) => {
  ...
  checkLimit = () => {
    ...
    axios
      .post(
        backendURI.url + "/supervisorrequests/checklimit/",
        { data: data },
        { headers: headers }
      )
      .then((res) => {
    ...
    return (
      <tr className="supervisor-details">
        <td>
          ...
          {!this.state.loading && (
            <span>
              {this.state.supervisorDetails.firstName}{" "}
              {this.state.supervisorDetails.lastName}
            </span>
          )}
        </td>
        <td>
          {!this.state.loading && (
            <span>{this.state.supervisorDetails.email}</span>
          )}
        </td>
        <td>
          {!this.state.loading && (
        ...
        <Modal isOpen={this.state.modal} toggle={this.toggle}>
          ...
          <ModalBody>
            <Table>
              ...
              <tbody>
                {!this.state.loading2 &&
                  this.state.projectIdList &&
                  this.state.projectIdList.length > 0 &&
                  this.state.projectIdList.map((projectId, key) => {
                    return (
                      <ProjectDetails
                        key={key}
                        projectId={projectId}
                        count={this.state.projectCount[key]}
                      />
                    );
                  })}
              </tbody>
            </Table>
            {!this.state.loading2 && this.state.projectIdList.length === 0 && (
              <span style={{ paddingLeft: "15px" }}>No any projects</span>
            )}
          </ModalBody>
        </Modal>
