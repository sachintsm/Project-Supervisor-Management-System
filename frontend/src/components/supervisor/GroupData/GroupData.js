...
class GroupData extends Component {
  ...
  componentDidMount = async () => {
    ...
    await axios
      .get(backendURI.url + "/users/supervisorsList", { headers: headers })
      .then((result) => {
        ...
        buttons: [
          {
            ...
            onClick: async () => {
              await axios
                .get(backendURI.url + "/users/student/" + index, {
                  headers: headers,
                })
                .then((res) => {
                  if (res.data.state === false) {
                    ...
                    axios
                      .post(
                        backendURI.url + "/createGroups/addStudentIndex",
                        data
                      )
                      .then((res) => {
                        if (res.data.state === false) {
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            for (let i = 0; i < this.state.selectedStaffList.length; i++) {
              ...
              await axios
                .post(
                  backendURI.url + "/createGroups/addSupervisorIndex",
                  data,
                  { headers: headers }
                )
                .then((res) => {
  ...
  viewFeedback = (project) => {
    this.props.history.push(
      "/supervisorhome/presentation/" + this.state.groupData._id,
      { groupDetails: this.state.groupData }
    );
  };
  viewChat = (project) => {
    this.props.history.push("/studenthome/chat/" + this.state.groupData._id, {
      groupDetails: this.state.groupData,
    });
  };
  biweeklyView = (project) => {
    this.props.history.push(
      "/supervisorhome/biweeklyview/" + this.state.groupData._id,
      { groupDetails: this.state.groupData }
    );
  };
  render() {
        ...
        <div className="container-fluid open-project open-project-background-color">
          ...
          <div className="gd-card">
            <div className="container gd-reg-head-div">
              <p className="gd-reg-head">
                Group - {this.state.groupData.groupId}
              </p>
            </div>
              ...
              <Row className="btn-row1" style={{ marginTop: "-70px" }}>
                <Col lg={4} md={4} xs={4} sm={12} className="btn-card-col1">
                  ...
                  >
                    <IconContext.Provider
                      value={{ className: "btn-icon1", size: "2em" }}
                    >
                      <div>
                ...
                <Col lg={4} md={4} xs={6} sm={12} className="btn-card-col1">
                  ...
                  >
                    ...
                    <span className="btn-title1">Meetings</span>
                  </Card>
                </Col>
                <Col lg={4} md={4} xs={6} sm={12} className="btn-card-col1">
                  <Card
                    className="btn-card1"
                    onClick={() => {
                      this.viewFeedback(this.state.project);
                    }}
                  >
                    <IconContext.Provider
                      value={{ className: "btn-icon1", size: "2em" }}
                    ...
                    <span className="btn-title1">Presentation Feedback</span>
                  </Card>
                </Col>
              </Row>
            </div>
