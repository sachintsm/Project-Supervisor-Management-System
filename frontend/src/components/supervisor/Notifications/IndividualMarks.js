...
class IndividualMarks extends Component {
  ...
  getProjectDetails = () => {
    ...
    axios
      .get(
        backendURI.url +
          "/projects/getproject/" +
          this.state.biWeekDetails.projectId,
        { headers: headers }
      )
      .then((res) => {
        this.setState({
    ...
    axios
      .get(
        backendURI.url +
          ...
          this.state.biWeekDetails.groupId,
        { headers: headers }
      )
      .then((res) => {
        this.setState({
  ...
  getIndividualMarks = () => {
    ...
    axios
      .get(
        backendURI.url +
          "/biweeksubmissions/individualmarks/" +
          this.state.biWeekId,
        { headers: headers }
      )
      .then((res) => {
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            if (!this.state.exists) {
              ...
              axios
                .post(
                  BackendURI.url + "/biweeksubmissions/individualmarks",
                  data,
                  { headers: headers }
                )
                .then((res) => {
      ...
      this.state.groupDetails.groupMembers.map((index) => {
        return (
          <StudentInfoCard
            ...
            existingMarks={this.state.existingMarks}
          />
        );
      });
        ...
        <div className="individual-marks">
          <Container className="div-container">
            <div className="title-div">
              {!this.state.loading2 && (
                <h4>
                  {this.state.projectDetails.projectYear}{" "}
              ...
              <h5>
                BiWeekly Report #{this.state.biWeekDetails.biweeklyNumber}{" "}
                Individual Marks
              </h5>
            </div>

            {!this.state.loading3 && this.state.exists && (
              <h6 className="marks-given-text">
                Note: Marks are Already Given
              </h6>
            )}
            {students}
            <div className="btn-div">
              {!this.state.loading && this.state.exists && (
                <Button
                  className="btn btn-info submit-btn mt-1"
