...
class GivePresentationFeedback extends Component {
    ...
    saveFeedback = () => {
        ...
        if(data.projectId && data.groupId && data.presentationName && data.feedback){
    ...
    handleDropdownChange = (e) => {
        let groupList = this.state.groups.filter(item=>item.projectId==e.target.value)
    ...
    render() {
        ...
        let projectList = this.state.projects.length > 0  && this.state.projects.map((item, i) => {
            return (
        ...
        let groupList = this.state.groupList.length > 0  && this.state.groupList.map((item, i) => {
            return (
                <option key={i} value={item._id}>{item.groupName} ( G{item.groupId} )</option>
            )
        }, this)
                            ...
                            <Card className="card-1">
                                {!this.state.loading1 && !this.state.loading2 && (
                                    <div>
                                        ...
                                        <div>
                                            <Input type="textarea" className="text-area" placeholder="Enter Presentaion Feedback" onChange={this.handleFeedbackChange}></Input>
                                        </div>

                                        <div>
                                            <button className="btn btn-info pg-bt mt-5 mb-3" style={{ width: '100%' }} onClick={this.saveFeedback}>Save Feedback</button>
