...
import BiweekData from "../../coordinator/ProjectData/BiweekData";
...
class ViewPresentationFeedback extends Component {
    ...
    getFeedbackList = async () => {
        ...
        await axios.get(BackendURI.url+"/presentationfeedback/get",{ headers: headers }).then(res=> {
            this.setState({
            ...
            buttons: [
                {
                    ...
                    onClick: async () => {
                        ...
                        await axios.delete(BackendURI.url+"/presentationfeedback/delete/"+item._id,{ headers: headers }).then(res=> {
                            this.setState({
                                successAlert: true
                            })
    ...
    render() {
        ...
        return (
            <React.Fragment>
                ...
                <div style={{padding: "0px 30px 20px 30px",backgroundColor: "white", marginTop: "30px",}}>
                    ...
                    <Table hover style={{ marginTop: 20,border: "1px solid #DDD"}} >
                        ...
                        <tbody>
                            {!this.state.loading && this.state.feedbackList.map((item,key)=> {
                                return(
                                    <tr key={key}>
                                        <td><ProjectName id={item.projectId}/></td>
                                        <td><GroupName id={item.groupId}/></td>
                                        <td><SupervisorName id={item.userId}/></td>
                                        <td>{item.presentationName}</td>
                                        <td>{item.feedback}</td>
                                        <td>
                                            <Row>
                                                {/*<Col><Create className="edit-btn" fontSize="large" onClick={() => this.onEditHandler(item)} /></Col>*/}
                                                <Col><DeleteForeverIcon className="del-btn" fontSize="large" onClick={() => this.onDeleteHandler(item)} /></Col>
                                            </Row>
                                        </td>
                                    </tr>
                                )
