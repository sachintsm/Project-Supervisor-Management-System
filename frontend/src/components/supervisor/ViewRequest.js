...
const Pending = React.memo((props) => (
  <tr>
    <td>
      {props.req.projectYear} {props.req.projectType} {props.req.academicYear}
    </td>
        ...
        <Button
          ...
          onClick={() =>
            props.sendAccept(
              props.req._id,
              props.req.groupId,
              props.req.supFirstName,
              props.req.supLastName,
              props.req.projectId
            )
          }
        >
    ...
    <td>
      <ButtonToolbar>
        <Button
          ...
          value="Mod"
          className="btn btn-info"
          onClick={() => props.sendReject(props.req._id, props.req.projectId)}
        >
          Reject
        </Button>
      </ButtonToolbar>
...
export default class ViewRequest extends Component {
  ...
  async componentDidMount() {
    ...
    axios
      .get(backendURI.url + "/users/getSupReq/" + userData.id)
      ...
      buttons: [
        {
          ...
          onClick: () => {
            ...
            console.log(req);
            axios
              .post(backendURI.url + "/users/updateReqState/" + id, obj)
              .then((res) => {
                ...
                if (res.data.state === true) {
                  ...
                  axios
                    .post(backendURI.url + "/users/getIndexNumbers/" + id, obj)
                    .then(async (response) => {
                      ...
                      if (this.state.emailA.length === 0) {
                        ...
                        for (var i = 0; i < this.state.emailA.length; i++) {
                          const email = await supervisorRequestEmail(
                            this.state.emailA[i],
                            group,
                            fName,
                            lName
  ...
  PendingList() {
    let k = false;
    let filteredReq = this.state.reqS.filter((currentReq) => {
      console.log(currentReq);
      return (
        currentReq.projectType
          .toLowerCase()
    ...
    if (k === true) {
      return filteredReq.map((currentReq, i) => {
        ...
        if (currentReq.state === "read" || currentReq.state === "pending") {
          return (
            <Pending
              sendAccept={this.reqSendAccept}
              sendReject={this.reqSendReject}
              req={currentReq}
              key={i}
            />
          );
