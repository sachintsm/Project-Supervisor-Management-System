...
class NoticeView extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (this.userType === "coordinator" || this.userType === "supervisor") {
      await axios
        .get(backendURI.url + "/notice/getNotice/" + userId)
        .then((res) => {
      ...
      for (let i = 0; i < this.state.noticeList.length; i++) {
        await axios
          .get(
            backendURI.url +
              "/notice/getProjectName/" +
              this.state.noticeList[i].projectId
          )
          .then((res) => {
      ...
      await axios
        .get(backendURI.url + "/notice/getNoticeByStudent/" + userId)
        .then((res) => {
      ...
      for (let i = 0; i < this.state.studentNoticeList.length; i++) {
        await axios
          .get(
            backendURI.url +
              "/notice/getProjectName/" +
              this.state.studentNoticeList[i].projectId
          )
          .then((res) => {
            ...
            var block = new studentNoticeBlock(
              _id,
              userType,
              userId,
              projectName,
            ...
            this.setState({
              studentNoticeListBlock: [
                ...this.state.studentNoticeListBlock,
                block,
              ],
            });
          });
    ...
    await axios.get(backendURI.url + "/notice/getAdminNotice").then((res) => {
      this.setState({
    ...
    if (this.userType === "coordinator") {
      return (
        <React.Fragment>
          ...
          <div className="container-fluid notice-background">
            <div className="container">
              <Tabs
                defaultActiveKey="Coordinators Notices"
                id="uncontrolled-tab-example"
                style={{ marginBottom: "30px", width: "100%" }}
              >
                <Tab
                  eventKey="Coordinators Notices"
                  title="Published By Coordinators"
                  className="tb-style"
                >
                        ...
                        return (
                          ...
                          >
                            <Row className="crd_notice-tittle-div">
                              <p className="crd_notice-name">
                                {types.noticeTittle}
                              </p>
                              <p className="crd_projects-name">
                                &nbsp;-&nbsp;{types.projectName}
                              </p>
                            </Row>

                            <Row className="crd_user-name-div">
                              <UserNameList id={types} />{" "}
                            ...
                            <div className="card-body">
                              <h6 className="crd_notice_content">
                                {types.notice}
                              </h6>
                              <a
      ...
      return (
        <React.Fragment>
          <Navbar panel={"admin"} />
          <div className="container-fluid admin-notice-background">
            <div className="container">
              {this.state.allNoticeList.length > 0 && (
                <div>
                  {this.state.allNoticeList.map((types) => {
