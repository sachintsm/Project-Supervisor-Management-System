    ...
    <td>
      ...
      <Button
        ...
        onClick={() =>
          props.send(
            props.pr._id,
            props.pr.academicYear,
            props.limit,
            props.pr.projectType,
            props.pr.projectYear
          )
        }
      >
        Set
...
export default class Academic extends Component {
      ...
      buttons: [
        {
          ...
          onClick: () => {
            ...
            axios
              .post(backendURI.url + "/users/setLimit", obj)
              .then((res) => {
  ...
  ProList1() {
    let filteredProjects = this.state.proS.filter((currentProj) => {
      return (
    ...
    return filteredProjects.map((currentProj, i) => {
      if (
        currentProj.isDeleted === false &&
        currentProj.projectState === true
      ) {
        return (
          <ProList
            pr={currentProj}
            key={i}
            limit={this.state.proL}
            hiden={this.hideModal}
            send={this.setPro}
            dis={this.state.dis}
            stat={this.state.show}
            desHan={this.handleChange}
            update={this.changePro}
            in={this.IncrementItem}
            de={this.DecreaseItem}
  ...
  async componentDidMount() {
    ...
    axios
      .get(backendURI.url + "/users/getSupProAca/" + userData.id)
      .then((response) => {
        ...
        for (var i = 0; i < l; i++) {
          ...
          axios
            .post(backendURI.url + "/users/getLimit/" + userData.id, ob)
            .then((res) => {
    ...
    return (
      <div className="container-fluid">
        ...
        <div className="row">
          <div
            className="col-md-12"
            style={{ backgroundColor: "#f8f9fd", minHeight: "1000px" }}
          >
