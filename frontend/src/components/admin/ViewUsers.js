...
const Staff = React.memo((props) => (
  <tr>
    ...
    <td className="table-body">
      <Row>
        <Col>
          <Link to={"/editprofile/" + props.staff._id}>
            <Create className="edit-btn" fontSize="default" />
          </Link>
        </Col>
        <Col>
          <DeleteForeverIcon
            className="del-btn"
            fontSize="default"
            onClick={() => props.delete(props.staff._id)}
          />
        </Col>
      </Row>
    </td>
...
const Students = React.memo((props) => (
  <tr>
    ...
    <td className="table-body">
      <Row>
        <Col>
          <Link to={"/editprofile/" + props.students._id}>
            <Create className="edit-btn" fontSize="default" />
          </Link>
        </Col>
        <Col>
          <DeleteForeverIcon
            className="del-btn"
            fontSize="default"
            onClick={() => props.delete(props.students._id)}
          />
        </Col>
      </Row>
    </td>
...
class ViewUsers extends Component {
  ...
  async componentDidMount() {
    ...
    if (!authState || !localStorage.getItem("isAdmin"))
      //check wether the user is logged in,if not re-directed to the login form
      this.props.history.push("/");

    axios
      .get(backendURI.url + "/users/get")
      .then((response) => {
  ...
  UserList3() {
    ...
    return filteredUsers.map((currentStaff, i) => {
      if (
        currentStaff.isStaff === true &&
        currentStaff.isCoordinator === true &&
        currentStaff.isDeleted === false
      ) {
        return <Staff delete={this.deleteUser} staff={currentStaff} key={i} />;
      } else return null;
    });
  ...
  UserList4() {
    ...
    return filteredUsers.map((currentStaff, i) => {
      if (
        currentStaff.isStaff === true &&
        currentStaff.isSupervisor === true &&
        currentStaff.isDeleted === false
      ) {
        return <Staff delete={this.deleteUser} staff={currentStaff} key={i} />;
      } else {
  ...
  UserList5() {
    let filteredUsers = this.state.userS.filter((currentStudents) => {
      return (
    ...
    return filteredUsers.map((currentStudents, i) => {
      if (
        currentStudents.isStudent === true &&
        currentStudents.isDeleted === false
      ) {
        return (
          <Students
      ...
      buttons: [
        {
          ...
          onClick: () => {
            axios
              .post(backendURI.url + "/users/deleteUser/" + data)
              .then((res) => {
