...
export default class registration extends Component {
  ...
  async componentDidMount() {
    ...
    await axios.get(backendURI.url + "/courseTypes/get").then((res) => {
        ...
        buttons: [
          {
            ...
            onClick: async () => {
              ...
              for (let i = 0; i < this.state.csvData.length - 1; i++) {
                  ...
                  .then(async (json) => {
                    if (json.state === false) {
                      ...
                      const email = await userRagistrationEmail(
                        this.state.csvData[i][2],
                        this.state.csvData[i][0],
                        this.state.csvData[i][1]
                      );
                      await axios
  ...
  validate = () => {
    ...
    if (
      this.state.form.courseType === "" &&
      this.state.form.userType !== "Admin" &&
      this.state.form.userType !== "Staff"
    ) {
      isError = true;
      errors.courseTypeError = "Student course must be specified *";
    }
    if (
      this.state.form.indexNumber.length === 0 &&
      this.state.form.userType !== "Admin" &&
      this.state.form.userType !== "Staff"
    ) {
      isError = true;
    ...
    if (
      this.state.form.regNumber.length === 0 &&
      this.state.form.userType !== "Admin" &&
      this.state.form.userType !== "Staff"
    ) {
      isError = true;
        ...
        buttons: [
          {
            ...
            onClick: () => {
              ...
              formData.append(
                "regNumber",
                this.state.form.regNumber.toLowerCase()
              );
                      ...
                      >
                        ...
                        <div style={{ marginBottom: "20px" }}>
                          {this.state.failedRegistration.length !== 0 && (
                            <p className="reg-error-title">
                              Following users does not registered to the
                              system..!
                            </p>
                          )}
                          ...
                          {this.state.failedRegistration.map((data) => {
                            return (
                              <div className="row">
                                <Col md="2" className="reg-error-msg">
                                  {data.firstName}
                                </Col>
                                <Col md="2" className="reg-error-msg">
                                  {data.lastName}
                                ...
                                <div className="form-group">
                                  ...
                                  <div className="form-group">
                                    ...
                                    <span className="course-reg">
                                      <Link to="/adminhome/registration/course">
                                        Course Registration
                                      </Link>
                                    </span>
                                  </div>
